...
class VisitorInterp implements Visitor {
	...
	private State validTransitionState	//État valide pour la transition de currentState avec userInput
	...
	override visitFSM(FSM f) {
		...
		while (userInput != "EXIT FSM"){
			...
			if (validTransitionState !== null){
				...
				System.out.println("Entry :" + currentState.getEntry())
	...
	//Il semblerait qu'il ne soit pas nécessaire de visiter les états pour interpréter.
	override visitState(State s) {
		if (s.isIsInitState()){
			if (currentState !== null)
				...
				currentState = s
				System.out.println("Initial entry: " + s.getEntry())
			}
		}
